...
public class WordGramTester {
	...
	@Test
	public void testToString() { //calls toString method and sees if output is correct String
		String[] words = {"apple", "zebra", "mongoose", "hat"};
		WordGram a = new WordGram(words,0,4);
		WordGram a2 = new WordGram(words,0,3);
		WordGram b2 = new WordGram(words,2,1);
		WordGram c = new WordGram(words,3,1);
		
		assertEquals("not a string", a.toString(), "apple zebra mongoose hat");
		assertEquals("not a string", a2.toString(), "apple zebra mongoose");
		assertEquals("not a string", b2.toString(), "mongoose");
		assertEquals("not a string", c.toString(), "hat");
	}
	
	@Test
	public void testShiftAdd(){ //calls shiftAdd method to see if words are shifted to correct place
		String[] words = {"apple", "zebra", "mongoose", "hat"};
		WordGram a = new WordGram(words,0,4);
		...
		assertEquals("shift failed on 0,4", a.shiftAdd("hello"), new WordGram(new String[]{"zebra","mongoose","hat","hello"},0,4));
		assertEquals("shift failed on 0,2", b.shiftAdd("pizza"), new WordGram(new String[]{"zebra", "mongoose", "pizza"},0,3));
		assertEquals("shift failed on 2,1", c.shiftAdd("computer"), new WordGram(new String[]{"computer"},0,1));
		assertEquals("shift failed on 3,1", d.shiftAdd("sleepy"), new WordGram(new String[]{"hat","sleepy"},0,2));
	}
}
